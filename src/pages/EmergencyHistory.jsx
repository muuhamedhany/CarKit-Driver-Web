    ...
    <div className="page-stack">
      ...
      {user?.cancellation_count > 3 && (
        <div className="banner warning status-banner">
          ...
          {t('High cancellation count:')} <strong>{user.cancellation_count}</strong>. {t('This may affect your eligibility.')}
        </div>
      )}
            ...
            <div className="table-row" key={row.request_id}>
              <div className="order-summary">
                ...
                <div className="order-copy">
                  <strong>{t(row.service_name) || t(row.service_type)}</strong>
              ...
              <div className="screen-actions">
                ...
                <span className={`status-pill ${sc.cls || ''}`}>{t(row.status)}</span>
              </div>
            </div>
          );
        })}
      ...
      {!filtered.length && (
        <div className="empty-state">
          ...
          <p>{t('No emergency jobs found for this filter.')}</p>
        </div>
      )}
    </div>
  );
