...
export default function DriverDashboard() {
  ...
  const load = async (showRefresh = false) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || t('Could not load dashboard.'));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    <div className="page-stack">
      ...
      {active && (
        <Link className="active-banner" to="/driver/active">
          ...
          <span>{t('Active order #')}{active.order_id}{t(' - tap to manage')}</span>
        ...
        {orders.map((order, i) => (
          ...
          >
            <div className="card-header-row">
              <div className="order-summary">
                ...
                <div className="order-copy">
                  <strong>{order.is_return ? `${t('Return Order #')}${order.order_id}` : `${t('Order #')}${order.order_id}`}</strong>
                  <p>{order.is_return ? `${order.item_count} ${t('items to return')}` : `${order.item_count} ${t('items ready')}`}</p>
                </div>
              </div>
              <span className={`status-pill ${order.is_return ? 'purple' : ''}`}>{order.item_count} {t('items')}</span>
            </div>
            ...
            <p className="meta-line">
              ...
              <span>
                {order.is_return
                  ? `${t('From:')} ${order.shipping_city || order.shipping_title || 'Customer'} ${t('to')} ${order.vendor_name || 'Vendor'}`
                  : `${t('To:')} ${order.shipping_city || order.shipping_title || 'Area unavailable'}`}
              </span>
            </p>
            <p className="meta-line">
            ...
            >
              <PackageCheck size={15} /> {order.is_return ? t('Accept Return') : t('Accept Order')}
            </button>
          </article>
        ))}
      </div>
      ...
      {!orders.length && (
        <div className="empty-state">
          ...
          <p>{t('No orders available right now. Check back soon.')}</p>
        </div>
      )}
    </div>
  );
